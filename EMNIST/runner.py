...
for i in range(NUM_SIMS):
    ...
    now = datetime.now()
    with open('results/fedmp_accuracies_alpha1.0_' + now.strftime("%Y%m%d%H%M") + '.txt', 'w') as fp:
            for item in test_acc:
                # write each item on a new line
                fp.write("%f\n" % item)
...
for i in range(NUM_SIMS):
    ...
    now = datetime.now()
    with open('results/pruneFL_accuracies_alpha1.0_' + now.strftime("%Y%m%d%H%M") + '.txt', 'w') as fp:
            for item in test_acc:
                # write each item on a new line
                fp.write("%f\n" % item)