...
class pruneFL_client(fl.client.Client):
    ...
    def get_channel_mask(self, num_filters=WIDTH):
        ...
        old_param_dict = deepcopy(maskmodel.state_dict())
        maskmodel.train()
        criterion = torch.nn.CrossEntropyLoss()
        ...
        for k in param_dict.keys():
            if k in ['conv1.weight','conv2.weight']:
                ...
                for w in range(num_filters):
                    ...
                    old_channel = old_param_dict[k][w]
                    gradient_norm = torch.norm(channel-old_channel,p=2)
                    channel_norms[w] = gradient_norm
        ...
        for k in param_dict.keys():
            if k not in OTHER_PARAMS:
                if 'conv1' in k or 'bn1' in k:
                    ...
                    drop_info[k] = []
                    flatten = list(range(WIDTH*7*7))
                    for i in mask2:
                        for j in flatten[i*7*7:(i+1)*7*7]:
