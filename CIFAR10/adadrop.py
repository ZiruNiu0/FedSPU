...
from adadrop_client import adadrop_client
import torch
import flwr as fl
import random
import numpy as np
...
from util import get_filters, get_parameters, set_filters, spu_aggregation, jaccard_distance, get_subnet, get_neuron_sets
CHANNEL = 3
Batch = 128
CLASSES = 10

class adadrop_strategy(fl.server.strategy.FedAvg):
    ...
    def aggregate_evaluate(self, server_round: int, results, failures):
        ...
        if self.evaluate_metrics_aggregation_fn:
            ...
            print(f"Adaptive Dropout: Round {server_round}, test accuracy = {metrics_aggregated['accuracy']}")
        elif server_round == 1:  # Only log this warning once
        ...
        if self.min_available_clients_ <= 100 and server_round == 1:
           ...
           jdf.to_csv('jaccard_hermes.csv', index=False)
...
def adadrop_client_fn(cid) -> adadrop_client:
  Epoch = 5
  drop_rate = get_rate(cid)
  dataset = cifar10Dataset("clientdata/cifar10_client_"+ str(cid) + "_ALPHA_0.1.csv")
  return adadrop_client(cid, dataset, drop_rate, Epoch, Batch)
